...
container_name="open-webui-theme-smoke-${GITHUB_RUN_ID:-local}-${GITHUB_RUN_ATTEMPT:-1}-$$"
container=""
...
for attempt in {1..20}; do
  ...
  case "$runtime_status:$health_status" in
    running:healthy)
      curl -fsS -o /dev/null --max-time 10 "http://127.0.0.1:${port}/"
      echo "Image health check and external HTTP probe passed: ${image}"
      exit 0
      ;;
    running:starting)
      echo "Waiting for image health check... attempt ${attempt}/20"
      sleep 3
      ;;
    running:missing)
      echo "Image does not define a Docker health check" >&2
      exit 1
      ;;
    *)
      echo "Image entered an unexpected state: ${state}" >&2
      exit 1
      ;;
  esac
done

echo "Image health check did not become ready: ${image}" >&2
exit 1
